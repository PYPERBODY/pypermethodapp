# PYPER Member Portal — Phase 2 Method Content Map

Phase 2 converts the complete patient-facing content from `previewv2.html` into the responsive Method library. The PDF-style cover and decorative divider pages are intentionally not rendered as member content.

| Portal chapter | Source guide sections |
|---|---|
| Start Here | The Member System |
| Medical & Member Safety | Medical & Member Safety |
| The PYPER Method | A body-first transformation system |
| Seven PYPER Pillars | The Seven PYPER Pillars |
| Three-Phase Standard | Three-Phase Standard |
| The Medication Window | What GLP-1s Do; Dose Rhythm, Tolerance & Questions |
| Body Composition | The Speed Problem; High-Protein Meal Framework |
| Metabolic Nutrition | The PYPER Nutrition Standard; Meal Structure & Decision Flow; Start Early, Not After Symptoms; Sample Weekly Meal Plan |
| Dining, Travel & Social Protocols | Eating Out; Travel Protocols; Events, Holidays & Family Meals |
| Supplements, Strength & Movement | Supplement Framework; The Body-Composition Backbone; Pilates + Low-Impact Cardio |
| Skin & Body Refinement | Skin + Body Changes During Weight Loss; The PYPER Body-Care Routine; PYPER Body-Care Pathways; Aesthetic Treatment Timing |
| Recovery, Mindset & Maintenance | Sleep, Stress & Cortisol; Nausea, Constipation & Hair Shedding; Mental Health + Body Image |
| Tracking, Safety & Red Flags | The Progress Dashboard; Red Flags |
| Beyond GLP-1 | Maintaining Beyond GLP-1; Member Journey Map; PYPER Check-In Dashboard; Member Support, Discord & Partner Perks |
| Appendices & Member Tools | Complete 5-Day Workout Program; Supplement Timing & Grocery List; Weekly Food & Symptom Logs; Workout & Measurement Trackers; Restaurant Card & Common Problems; Emergency Meal Ideas |

## Phase 2 UX conversion

- Fixed PDF pages are replaced with responsive chapters and expandable sections.
- Guide tables include mobile card labels and collapse into readable cards on narrow screens.
- Every chapter includes save, provider-question, and completion placeholders.
- Every chapter displays related tracker and reminder links.
- The persistent Safety tab remains available outside the Method library.
- Internal design notes and patient-inappropriate swatch pages are not included.
